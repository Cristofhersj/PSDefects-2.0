...
def start_capture():
    global cameras, root, Captura, reference_images, start_time
    ...
    start_time = time.time()  # Record start time of capture
...
def stop_capture():
    ...
    print("Total capture duration: {:.2f} seconds".format(time.time() - start_time))
    start_button.config(state=NORMAL)  # Re-enable start button after stopping the capture
    root.destroy()
    ...
    for i, camera in enumerate(cameras):
        ...
        try:
            ...
            if grabResult.GrabSucceeded():
                ...
                processed_display_image = cv2.resize(processed_image, (480, 480))
...
def process_contours(img1, reference, ksize=3, min_defect_size=50):
    ...
    # Process contours
    for c in contours:
        # Check if the contour area is larger than min_defect_size
        if cv2.contourArea(c) > min_defect_size:
